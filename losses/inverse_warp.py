...
def depth2world(depth, intrinsics):
    """
    depth: (B, 1, H, W)
    intrinsics: (B, 3, 3)
    """
    return depth_to_3d(depth, intrinsics)
...
def inverse_warp(img, depth, ref_depth, pose, intrinsics, padding_mode='zeros'):
    ...
    world_points = depth2world(depth, intrinsics) # B 3 H W
    ...
    # projected_img2 = F.grid_sample(img, pix_coords, padding_mode="border", align_corners=False)
    projected_img = projected_img * valid_mask
    projected_depth = F.grid_sample(ref_depth, pix_coords, padding_mode=padding_mode, align_corners=False)

    return projected_img, projected_img, projected_depth, computed_depth
...
def inverse_rotation_warp(img, rot, intrinsics, padding_mode='zeros'):
    ...
    world_points = depth2world(torch.ones(B, 1, H, W).type_as(img), intrinsics) # B 3 H W
